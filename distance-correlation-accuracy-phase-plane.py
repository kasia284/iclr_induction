...
CHECKPOINTS (morphology-grid-evolution-layers-vs-seqlen.py), we get a
...
gridness-vs-accuracy-scatter.py's static scatter.

Two geometry metrics, both computed from per-layer class means at each
checkpoint (LAYERS, from morphology-grid-evolution-layers-vs-seqlen.py --
...
families (see gridness-vs-accuracy-scatter-interactive-thumbnails-
morphology-numbers.py) -- where the negative final-DC-vs-accuracy
...
cache (needs morphology-grid-evolution-layers-vs-seqlen.py's SWEEP_KEYS
...
assign_families from gridness-vs-accuracy-scatter.py (imported via
importlib since their filenames aren't valid module identifiers).
"""
import base64
