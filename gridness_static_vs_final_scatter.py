...
quantity as morphology_distance_correlation_sweep.py, but computed for
...
morphology_final_dirichlet_energy_sweep.py's companion metric (that script
uses Dirichlet energy; this one uses distance correlation instead), read
via load_final_layer_class_means from activation_unembedding_dot_product.py.
...
real_embeddings_nearest_neighbors.py's load_embedding_only_model), which is
