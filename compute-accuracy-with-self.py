...
starting-geometry-vs-accuracy-scatter.py / -ring.py, under an ALTERNATIVE
...
on a GPU machine, then re-run starting-geometry-vs-accuracy-scatter.py
--with-self / starting-geometry-vs-accuracy-scatter-ring.py --with-self
(which just read these new cache files) wherever's convenient afterward.

Usage:
    python compute-accuracy-with-self.py                # both grid + ring word lists
    python compute-accuracy-with-self.py --grid-only
    python compute-accuracy-with-self.py --ring-only
"""
...
scatter_mod = _load_module("starting_geometry_scatter", "starting-geometry-vs-accuracy-scatter.py")

RING_SWEEP_KEYS = ring_mod.RING_SWEEP_KEYS


def grid_keys():
    """The same word lists starting-geometry-vs-accuracy-scatter.py sweeps
