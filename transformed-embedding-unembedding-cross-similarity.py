"""Applies the fitted ridge regression map (embedding-to-unembedding-
linear-fit.py's W_ridge, b_ridge -- picked by held-out R^2 over a vocab-
...
distance) as embedding-unembedding-cross-similarity.py, but between the
...
Requires embedding-to-unembedding-linear-fit.py to have been run first
...
def main():
    ...
    if not os.path.exists(FIT_CACHE_PATH):
        raise RuntimeError(
            f"No cached fit at {FIT_CACHE_PATH} -- run embedding-to-unembedding-linear-fit.py first."
        )
