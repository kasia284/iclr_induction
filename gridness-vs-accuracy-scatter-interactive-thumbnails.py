"""Interactive version of gridness-vs-accuracy-scatter-with-thumbnails.py:
...
Reuses thumbnail PNGs already rendered by gridness-vs-accuracy-scatter-
with-thumbnails.py (results/reproduce/plots/gridness_vs_accuracy/
thumbnails/{key}.png), rendering any that are missing. Both companion
scripts are imported via importlib since their filenames aren't valid
module identifiers. Pure cache reads + thumbnail rendering, no GPU/model
...
thumb_mod = _load_module("gridness_vs_accuracy_thumbnails", "gridness-vs-accuracy-scatter-with-thumbnails.py")
