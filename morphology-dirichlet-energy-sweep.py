...
01_reproduce.py's MORPHOLOGY_SWEEP_KEYS), computes the Dirichlet energy of
the permutation's grid graph using its STATIC embeddings -- the token
embeddings straight out of the embedding layer (W_E), before any forward
pass / context -- as node features, via utils.compute_dirichlet_energy.

Low Dirichlet energy = grid-adjacent words already sit close together in
the raw embedding space, before the model has done anything with context.
Since the model's own morphological/lexical prior lives in W_E, this is a
purely static (no forward pass, no GPU) proxy for the same "does grid
adjacency align with the model's pretrained prior" question the same-lemma
edge fraction targets -- but measured directly in embedding space instead
of by lemma-matching.
...
continuous scale by their permutation's Dirichlet energy instead of by
distinct qualitative colors.

Reads W_E via the same lightweight safetensors-only loader as
real-embeddings-nearest-neighbors.py (imported via importlib since its
filename isn't a valid module identifier), and token ids via
to_single_token from activation-unembedding-dot-product.py. Accuracy
...
def compute_static_dirichlet_energy(W_E, tokenizer, word_list_key):
    """Dirichlet energy of word_list_key's grid graph, using each word's raw
    W_E embedding (before any forward pass) as its node feature."""
    ...
    return compute_dirichlet_energy(embeddings, adjacency)


def plot_accuracy_by_dirichlet_energy(accs_by_key, de_by_key, out_dir,
                                       filename="accuracy_curve_sweep_by_dirichlet_energy"):
    """Like 01_reproduce.py's plot_accuracy_comparison, but each permutation's
    line is colored on a continuous scale by its static-embedding Dirichlet
    energy (de_by_key), with a colorbar, instead of a fixed qualitative
    palette."""
    ...
    fig.colorbar(cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax,
                 label="Dirichlet energy (static W_E)")
    ...
    pfig.update_layout(**plotly_line_layout(
        "Accuracy vs sequence length, colored by static-embedding Dirichlet energy",
...
def main():
    ...
    for key in MORPHOLOGY_SWEEP_KEYS:
        de_by_key[key] = compute_static_dirichlet_energy(W_E, tokenizer, key)
        print(f"{key}: Dirichlet energy = {de_by_key[key]:.4f}")

    accs_by_key = {k: load_cached_accuracies(k) for k in MORPHOLOGY_SWEEP_KEYS}
    plot_accuracy_by_dirichlet_energy(accs_by_key, de_by_key, PLOTS_DIR)
