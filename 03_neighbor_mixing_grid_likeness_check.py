...
03_neighbor_mixing_synthetic_grid_transformed.py rather than duplicating
them (loaded via importlib, same convention used throughout this repo for
...
spec = importlib.util.spec_from_file_location(
    "synth_mod", os.path.join(REPO, "03_neighbor_mixing_synthetic_grid_transformed.py")
)
...
def main():
    ...
    disparities = []
    for r in rounds:
        projected, _ = synth_mod.pca_2d(embs_by_round[r])  # indexed by node
    ...
    for ax, r in zip(axes_flat, panel_rounds):
        projected, _ = synth_mod.pca_2d(embs_by_round[r])
        projected = projected.numpy()
