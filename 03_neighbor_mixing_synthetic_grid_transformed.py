...
Generalizes 03_neighbor_mixing_synthetic_grid_permuted.py: instead of lifting
...
def plot_round_on_ax(ax, projected, grid, labels, title):
    draw_class_mean_pca_on_ax(ax, grid, projected, words=labels, word_to_color=LABEL_TO_COLOR,
                               marker_size=100, label_fontsize=6, label_offset=(4, 4), is_3d=False)
    ax.set_xlabel("PC1")
    ax.set_ylabel("PC2")
...
def main():
    ...
    for ax, r in zip(axes_flat, ROUNDS):
        projected, _ = pca_2d(embs_by_round[r])
