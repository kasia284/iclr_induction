"""Ring-task analogue of starting_geometry_vs_accuracy_scatter.py: scatter
...
gridness_vs_accuracy_scatter_ring.py's load_real_full_context_accuracy.
...
safetensors-only loader as real_embeddings_nearest_neighbors.py, and token
ids via to_single_token from activation_unembedding_dot_product.py. Family
coloring (assign_families) is reused from gridness_vs_accuracy_scatter.py
(topology-agnostic). CPU-only, no GPU / new model forward passes.

Usage:
    python starting_geometry_vs_accuracy_scatter_ring.py
    ...
    # compute_accuracy_with_self.py first (needs a GPU) to produce it.
    # Output filenames get a "_with_self" suffix.
    python starting_geometry_vs_accuracy_scatter_ring.py --with-self
"""
    ...
    for key in RING_SWEEP_KEYS:
        ...
        if os.path.exists(acc_path):
            ...
            print(f"Skipping {key}: missing {os.path.basename(acc_path)} "
                  f"(run {'compute_accuracy_with_self.py' if with_self else '01_reproduce_ring.py'} "
                  f"with this key first).")
    return keys


def load_real_full_context_accuracy(word_list_key, with_self=False):
    """Mirrors gridness_vs_accuracy_scatter_ring.py's function of the same
    name; with_self=True reads the include_self=True variant instead
    (accuracies_Ring_{key}_with_self.npz, from compute_accuracy_with_self.py)."""
...
def plot_scatter(x_by_key, acc_by_key, keys, out_dir, x_label, title_prefix, filename):
    """Mirrors starting_geometry_vs_accuracy_scatter.py's plot_scatter."""
...
def main():
    ...
    if not sweep_keys:
        ...
        producer = "compute_accuracy_with_self.py" if with_self else "01_reproduce_ring.py"
