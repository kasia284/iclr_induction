...
script in this repo uses (morphology_distance_correlation_sweep.py,
gridness_static_vs_final_scatter.py, permutation_graph_similarity.py):
...
random-walk sequences) -- same quantity as gridness_vs_accuracy_scatter.py's
y axis, just plotted here against the STARTING geometry's conflict instead
of the FINAL-LAYER geometry's alignment.

Usage:
    python starting_geometry_vs_accuracy_scatter.py
    ...
    # accuracies_Grid_{key}.npz -- run compute_accuracy_with_self.py
    ...
    python starting_geometry_vs_accuracy_scatter.py --with-self
...
distance_correlation_accuracy_phase_plane.py) -- i.e. each base key plus
...
same lightweight safetensors-only loader as real_embeddings_nearest_neighbors.py
(imported via importlib, consistent with how sibling scripts in this repo
share helpers), and token ids via to_single_token from
activation_unembedding_dot_product.py. Family coloring (assign_families)
is reused from gridness_vs_accuracy_scatter.py. CPU-only, no GPU / new
...
def load_real_full_context_accuracy(word_list_key, with_self=False):
    ...
    cached random-walk sequences. Mirrors gridness_vs_accuracy_scatter.py's
    function of the same name, but can read the include_self=True variant
    (accuracies_Grid_{key}_with_self.npz, from compute_accuracy_with_self.py)
    ...
    for key in family_keys():
        ...
        if os.path.exists(acc_path):
            ...
            print(f"Skipping {key}: missing {os.path.basename(acc_path)} "
                  f"(run {'compute_accuracy_with_self.py' if with_self else '01_reproduce.py'} "
...
def plot_scatter(x_by_key, acc_by_key, keys, out_dir, x_label, title_prefix, filename):
    """Mirrors gridness_vs_accuracy_scatter.py's plot_scatter, just with a
...
def main():
    ...
    if not sweep_keys:
        ...
        producer = "compute_accuracy_with_self.py" if with_self else "01_reproduce.py"
