...
distance_correlation_accuracy_phase_plane.py. For a fixed word list, as
context length k grows through the log-spaced CHECKPOINTS (ring_evolution_seqlen.py), we get a sequence of points (metric(k), accuracy(k));
...
layer 01_reproduce_ring.py / ring_evolution_seqlen.py track) at each
...
ring_evolution_seqlen.py, a GPU run) and an accuracies_Ring_*.npz cache
...
ring_evo_mod = _load_module("ring_evolution", "ring_evolution_seqlen.py")
    ...
    for key in RING_SWEEP_KEYS:
        ...
        if result is None:
            print(f"Skipping {key}: missing ring_evolution_seqlen.npz "
                  f"(run ring_evolution_seqlen.py, on a GPU) or accuracies cache.")
            continue
        curves[key] = result
